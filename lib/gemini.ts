  ...
  for (const model of models) {
    try {
      ...
      if (!response.ok) {
        ...
        if (status === 429 || status >= 500) {
          lastError = new Error(`Gemini error (${model}): ${status} - ${errorText}`);
          console.warn(`[Gemini Image] Model ${model} failed with ${status}. Falling back...`);
          continue;
        }
      ...
      lastError = err instanceof Error ? err : new Error(String(err));
      console.warn(`[Gemini Image] Model ${model} failed with error: ${lastError.message}. Falling back...`);
    }
  }
